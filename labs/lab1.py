В вашем коде есть несколько ошибок. Давайте исправим их:

1. В функции `check_point_in_figure` вы используете `self.entry_x` и `self.entry_y`, но они не являются атрибутами класса `GraphWindow`. Вам нужно передать их как аргументы в эту функцию.

2. В той же функции `check_point_in_figure` у вас два `return` подряд, из-за чего второй `return` никогда не будет выполнен. Вам нужно поместить второй `return` в блок `else`.

3. В функции `check_point_in_figure` в блоке `try` вам нужно вызывать метод `check_point_in_figure` из экземпляра класса, используя `self`.
